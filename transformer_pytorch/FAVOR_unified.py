...
def orthogonal_matrix_chunk(cols, device=None):
    ...
    #q, r = torch.qr(unstructured_block.cpu(), some=True) ## updated
...
def all_modality_causal_linear_attn_cuda(q, k, v, condition_len, eps=1e-6):
    # from fast_transformers.causal_product import CausalDotProduct ## Removed for dependency fix
    ...
    # causal_dot_product_fn = amp.float_function(CausalDotProduct.apply) if is_half else CausalDotProduct.apply  ## Removed for dependency fix
    ...
    with cuda_context():
        ...
        #out = causal_dot_product_fn(q, k, v)  ## Removed for dependency fix
        out = F.scaled_dot_product_attention(q, k, v, is_causal=True)  ## added for dependency fix
